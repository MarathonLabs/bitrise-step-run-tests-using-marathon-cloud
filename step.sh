...
if [[ $OSTYPE == 'darwin'* ]]; then
  ...
  docker run -v "$BITRISE_APK_PATH":/work/app.apk -v "$BITRISE_TEST_APK_PATH":/work/appTest.apk -it --rm $docker_image -api-key "$api_key" -apk ./app.apk -testapk ./appTest.apk 
fi
